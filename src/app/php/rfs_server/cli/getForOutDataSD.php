...
function getForOutDataSD($db,$cd) {
...
, shisetsu.encho
, shisetsu.seiri_no 
, sd.yokokuhyoushiki_umu
, CASE sd.yokokuhyoushiki_umu 
  WHEN 0 THEN '不明' 
  WHEN 1 THEN '有' 
  WHEN 2 THEN '無' 
  ELSE '' 
  END yokokuhyoushiki_umu_str
, sd.jouhouban_umu
, CASE sd.jouhouban_umu
  WHEN 0 THEN '不明' 
  WHEN 1 THEN '有' 
  WHEN 2 THEN '無' 
  ELSE '' 
  END jouhouban_umu_str
, sd.anzentou_syoumeitou
, sd.syadanji_riyuu
,sd.d_hokuden_kyakuban
,sd.d_keiyaku_houshiki
,sd.d_kaisen_id
,sd.d_kaisen_kyakuban
,sd.d_hikikomi
,sd.d_denki_dai
,sd.d_denki_ryou
,sd.bikou
...
,sd.update_dt
,sd.update_account
,sd.update_account_nm
,sd.update_busyo_cd
FROM
s_tmp shisetsu 
LEFT JOIN rfs_t_daichou_sd sd 
  ON shisetsu.sno = sd.sno 
...
function outputCSVSD($data,$csvDirName,$csv_kihon_header) {
  $csv_daichou_header = [
