...
function getForOutDataBH($db,$cd) {
...
, CASE bh.heating
  ...
  ELSE '' 
  END heating_umu
,bh.d_hokuden_kyakuban
,bh.d_keiyaku_houshiki
,bh.d_kaisen_id
,bh.d_kaisen_kyakuban
,bh.d_hikikomi
,bh.d_denki_dai
,bh.d_denki_ryou
,bh.bikou
...
,bh.update_dt
,bh.update_account
,bh.update_account_nm
,bh.update_busyo_cd
FROM
s_tmp shisetsu 
LEFT JOIN rfs_t_daichou_bh bh 
  ON shisetsu.sno = bh.sno 
...
function outputCSVBH($data,$csvDirName,$csv_kihon_header) {
  $csv_daichou_header = [
  'kaidan'=>'階段'
  ,'kaidan_umu'=>'階段有無'
  ,'slope'=>'スロープ'
