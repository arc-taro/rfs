 ...
 */
class DaichouMultisheetExcelWrapper {
...
*/
     ...
     * @param string $format ファイル形式（DaichouMultisheetExcelWrapper::FORMAT_XLS / DaichouMultisheetExcelWrapper::FORMAT_XLSX）。空なら拡張子から推測
...
// UPD 20150510 haramoto 入力画像の形式ごとに開く方法を変更 -->
                                    ...
                                    switch($image_type) {
                                        ...
                                        case IMAGETYPE_BMP:
                                            $bmp_path=$file1;
                                            $png_path=mb_substr($file1,0,mb_strlen( $file1, "UTF-8" )-4).".png";
                                            // BMPは処理できないのでimagemagicでpngにする。COLOR関係でエラーが出てるけど。
                                            log_message("debug",print_r($png_path,true));
                                            exec("/usr/bin/convert " . $bmp_path  . " " . $png_path );
                                            $in = imagecreatefrompng($png_path);
                                            //$in = $this->ImageCreateFromBMP($file1);
                                            break;
                                        case IMAGETYPE_TIFF_II:
                                            $tif_path=$file1;
                                            $png_path=mb_substr($file1,0,mb_strlen( $file1, "UTF-8" )-4).".png";
                                            // tifは処理できないのでimagemagicでpngにする。COLOR関係でエラーが出てるけど。
                                            log_message("debug",print_r($png_path,true));
                                            exec("/usr/bin/convert " . $tif_path  . " " . $png_path );
                                            $in = imagecreatefrompng($png_path);
                                            //$in = $this->ImageCreateFromBMP($file1);
                                            break;
                                        }
...
// UPD 20111102 nakamura 判定間違い -->
                                    ...
                                    if( $oWidth >= $oHeight ){
                                      // 横長
                                      $w=$match2[2];
                                      // $oHeight*($match2[2]/$oWidth)
                                      $h=$oHeight*$pWidth;
                                      // この縦がExcelより長い場合はさらに縮小しなきゃ
                                      if ($h>$match2[3]) {
                                        // 縮小率$match2[3]/$h
                                        $w=$w*($match2[3]/$h);
                                        $h=$match2[3];
                                      ...
                                      // 縦長
                                      // $oWidth*($match2[3]/$oHeight);
                                      $w=$oWidth*$pHeight;
                                      $h=$match2[3];
                                      // この幅がExcelより長い場合はさらに縮小しなきゃ
                                      if ($w>$match2[2]) {
                                        // 縮小率$match2[2]/$w
                                        $w=$match2[2];
                                        $h=$h*($match2[2]/$w);
                                    ...
                                    // そのまま入れるとなんだか横長になるので幅を少し短くする
                                    // そのまま入れるとなんだかはみ出るので、全体を縮小
                                    $objDrawing->setWidth($w*0.9*0.95);
                                    $objDrawing->setHeight($h*0.95);
                                    $ofsx = 0;
                                    $ofsy = 0;
/*
                                    // 縦横比固定にしない
                                    ...
                                    if( $pWidth <= $pHeight ){
                                        ...
                                        $objDrawing->setWidthAndHeight($match2[2],$oHeight * $pWidth);

										// $oWidth:$oHeight=$match2[2]:$match2[3]
                                        ...
                                        $objDrawing->setWidthAndHeight($oWidth * $pHeight,$match2[3]);

                                        //$objDrawing->setWidth($oWidth * $pHeight);
                                        //$objDrawing->setHeight($match2[3] * 0.877);
                                        // 横方向が余るのでオフセットを計算
