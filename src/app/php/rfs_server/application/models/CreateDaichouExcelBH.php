...
// 立体横断登録用
class CreateDaichouExcelBH extends CreateDaichouExcel {
    ...
    */
    protected function getDaichouInfo($sno, $shisetsu_kbn,$daichou_tbl){
        ...
        $fields= <<<EOF
CASE WHEN kaidan = 1 THEN '有'
  WHEN kaidan = 2 THEN '無'
  WHEN kaidan = 0 THEN '不明' END kaidan_str
, CASE WHEN slope = 1 THEN '有'
  WHEN slope = 2 THEN '無'
  WHEN slope = 0 THEN '不明' END slope_str
, CASE WHEN oshiage = 1 THEN '有'
  WHEN oshiage = 2 THEN '無'
  WHEN oshiage = 0 THEN '不明' END oshiage_str
, CASE WHEN heating = 1 THEN '有'
  WHEN heating = 2 THEN '無'
  WHEN heating = 0 THEN '不明' END heating_str
, d_hokuden_kyakuban
, d_keiyaku_houshiki
, d_hikikomi
, d_denki_dai
